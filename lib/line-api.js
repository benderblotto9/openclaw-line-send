 ...
 */
export function getRetryAfter(res) {
...
/**
 * Exponential backoff with equal jitter, capped at MAX_WAIT_MS.
 * Guarantees at least half the capped backoff, randomizing the rest, so
 * concurrent retries don't all wake up and hammer the API in lockstep.
 */
export function backoffWithJitter(attempt) {
  const capped = Math.min(BASE_BACKOFF_MS * Math.pow(2, attempt), MAX_WAIT_MS);
...
async function lineRequest(token, method, path, body = null, opts = {}) {
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    ...
    try {
      ...
      // Cap either way so a large server-specified value can't block the CLI.
